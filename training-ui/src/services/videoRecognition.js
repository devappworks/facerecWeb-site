...
export const videoRecognitionService = {
  ...
  async uploadSync(file, domain = 'serbia', intervalSeconds = 3.0) {
    ...
    try {
      ...
      const response = await api.post('/api/video/upload', formData, {
        ...
        timeout: 600000, // 10 minutes for sync video upload (processing + upload)
      })
  ...
  /**
   * Get list of all videos from backend
   * Returns all uploaded videos with their processing status
   */
  async getAllVideos() {
    if (USE_MOCK_DATA) {
      return {
  ...
  /**
   * Delete a video and all associated files (frames, results)
   */
